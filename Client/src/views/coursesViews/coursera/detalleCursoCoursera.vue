  ...
  <div class="bg-gray-100 p-4 pb-4 md:p-0 md:pb-6 max-w-screen-full">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 max-w-screen-xl mx-auto">
      ...
      <main class="md:col-span-2">
        ...
        <section class="hidden md:block bg-white p-4 md:px-6 rounded mb-6 justify-between items-center">
          <seo-component
            :title="pageTitleSEO"
            :description="pageDescriptionSEO"
            :breadcrumbs="breadcrumbs"
          />
          <h1 class="md:text-3xl font-bold text-gray-800">
            {{ course.title }}
          </h1>
        ...
        <section class="bg-white p-4 md:p-6 rounded mb-6">
          <!-- Resumen del curso con botón de expandir -->
          <h2 class="text-xl mb-2 font-bold">Resumen de este curso</h2>
          <div :class="{ 'max-h-48 overflow-hidden': !expandDescription }" class="relative"
            v-html="course.formattedDescription"></div>
        ...
        <div class="bg-white p-4 md:p-6 rounded mb-6">
          <h2 class="text-xl mb-2 font-bold">Información institucional</h2>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white">
            ...
            <section class="bg-white p-4 md:p-6 rounded flex-col justify-center">
              ...
              <div class="relative">
                <p>
                  Nuestros educadores poseen habilidades especializadas en la
                  enseñanza a distancia y comparten una profunda pasión por
                  educar, comprometiéndose a mantenerse continuamente
                  actualizados para ofrecer lecciones completas y de elevado
                  estándar académico.
                </p>
              </div>
            </section>
            ...
            <section class="bg-white p-4 md:p-6 rounded flex-col justify-center">
              ...
              <div class="relative">
                <p>
                  Adquiere saberes de la mano de educadores altamente
                  experimentados en educación a distancia, comprometidos con una
                  actualización constante para proporcionar clases de excelencia
                  académica. Contamos con una sólida trayectoria en la enseñanza
                  en línea, respaldada por la dedicación y la experiencia de
        ...
        <section class="bg-white p-4 md:p-6 rounded">
          <h3 class="text-xl mb-2 font-bold">Instructores</h3>
      ...
      <div class="order-first md:order-last bg-white p-4 md:p-6 mb-auto rounded">
        ...
        <!-- Título y Universidad -->
        <section class="md:hidden text-right items-end ml-auto rounded-lg md:ml-0">
        ...
        <div class="md:hidden bg-white md:p-6 rounded justify-between items-center">
          ...
          <div class="p-2 text-md">
            <ul>
              <!-- Institución -->
              <li class="flex text-sm text-gray-800 py-2 border-b border-gray-100">
                <div class="flex items-center">
                  ...
                  <a href="/cursos/Coursera">
                    <button class="flex hover:underline transition"
                      title="Estos cursos provienen de nuestro proveedor: Coursera">
                      <span>Coursera - {{ course.university }}</span>
                      <svg class="ml-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" fill="#000000">
                        <path d="M0 0h24v24H0V0z" fill="none" />
                        <path
              ...
              <li class="flex text-sm text-gray-800 py-2 border-b border-gray-100">
                <div class="flex items-center">
                  ...
                  <span>{{ course.mainCategory }} -
                    {{ course.secondaryCategory }}</span>
                </div>
              </li>
              <!-- Nivel -->
              <li class="flex text-sm text-gray-800 py-2 border-b border-gray-100">
              ...
              <!-- Precio -->
              <li class="flex text-sm text-gray-800 py-2">
                ...
                <span class="text-sm">{{
        course.price || "Inscripción Gratuita"
      }}</span>
              </li>
            </ul>
        </div>
        ...
        <div @click="redirectToCoursera"
          class="cursor-pointer flex bg-indigo-600 text-white text-center py-2 px-4 rounded hover:bg-indigo-800 transition-colors duration-300 ease-in-out w-full items-center text-md justify-center font-semibold">
          Ir al curso
...
const pageDescriptionSEO = computed(() => {
  let description =
    course.value.shortDescription &&
      course.value.shortDescription !== "No short description found"
      ? course.value.shortDescription
...
const breadcrumbs = computed(() => {
  ...
  return [
    { text: "Inicio", to: "/", active: route.path === "/" },
    {
      text: "Coursera",
      to: "/cursos/coursera",
      active: route.path.includes("/cursos/coursera"),
    },
    {
      text: course.value.title,
      to: `/coursera/cursos/${courseSlug}`,
      active: true,
    },
  ];
});
...
function formatDescription(description) {
  // Divide el texto en párrafos basado en un patrón. Por ejemplo, dividirlo cada vez que se encuentre un punto seguido de un espacio y una letra mayúscula.
  const paragraphs = description.split(/(?<=\.|\d\.)\s+(?=[A-Z]|\d)/);

  return paragraphs
    .map((paragraph) =>
      // Elimina espacios extras y asegura que el párrafo no esté vacío
      paragraph.trim().length > 0 ? `<p>${paragraph}</p>` : ""
    )
    .join("");
}
...
onMounted(async () => {
  ...
  try {
    ...
    if (response.data) {
      ...
      course.value.formattedDescription = formatDescription(
        course.value.longDescription
      );

      // No olvides actualizar el SEO si es necesario
      updateSEO();
