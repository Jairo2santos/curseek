<template>
  ...
  const loadProfessors = async () => {
    try {
      const response = await axios.get('/api/cursos/coursera/universidad/UDeLosAndesCo/profesores');
  ...
  const loadCourses = async () => {
    ...
    try {
      const response = await axios.get(`/api/cursos/coursera/universidad/UDeLosAndesCo?page=${currentPage.value}`);
